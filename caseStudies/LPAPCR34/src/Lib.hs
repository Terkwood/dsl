...
data Stm = Section TermExpr -- first TermExpr should always be an NL expression (:@)
         ...
         | CRule { rulename :: TermExpr
                 , item :: TermExpr
                 , body :: TermExpr -- UnOp ISA x    vs    UnOp HAS x
                 , when :: Maybe TermExpr
                 }
  deriving (Show, Eq)
...
type MkDefs = TermExpr -> TermExpr -> TermExpr -> Stm
(§.=§) :: MkCRule
(§.=§) a b = CRule a (UnaOp HasAttr b) Nothing
...
superSimple1 =
  ...
  ("superSimple1" 💬 "Super Simple Whiteboard Example") -- rulename
  ("Business" 💬 "Business")                            -- entity
  ("is_legal" 💬 "legal")                               -- property / attribute
someFunc :: String -> IO ()
someFunc myinput = do
